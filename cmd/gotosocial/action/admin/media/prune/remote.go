...
// Remote prunes orphaned remote from storage.
var Remote action.GTSAction = func(ctx context.Context) error {
	...
	pruned, err := manager.PruneAllRemote(ctx, config.GetMediaRemoteCacheDays(), dry)
	if err != nil {
		return fmt.Errorf("error pruning: %s", err)
	}

	if dry /* dick heyyoooooo */ {
		log.Infof("DRY RUN: %d remote media attachments are eligible to be pruned", pruned)
	} else {
		log.Infof("%d stored remote media attachments were orphaned and pruned", pruned)
	}
