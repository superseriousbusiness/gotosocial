...
func (s *Surface) UpdateConversationsForStatus(ctx context.Context, status *gtsmodel.Status, notify bool) error {
	...
	for _, participant := range allParticipantsSet {
		...
		statusAuthoredByConversationOwner := status.AccountID == localAccount.ID

		if conversation == nil {
			// Create a new conversation starting with this status.
			conversation = &gtsmodel.Conversation{
				...
				LastStatus:       status,
				Read:             util.Ptr(statusAuthoredByConversationOwner),
			}
		} else {
			// Update the existing conversation.
			// If there is no previous last status or this one is newer, set it as the last status.
			// TODO: (Vyr) does it make more sense for the most recently *created* status or the most recently *received* status to be the last status?
			...
			conversation.Read = util.Ptr(*conversation.Read && !statusAuthoredByConversationOwner)
		}

		// Create or update the conversation.
		err = s.State.DB.PutConversation(ctx, conversation, "last_status_id", "read")
		if err != nil {
			log.Errorf(
				ctx,
