...
func (p *Processor) GetAll(
	...
	for _, conversation := range conversations {
		// Check that at least one account in this conversation is visible. If not, skip the conversation.
		atLeastOneAccountVisible := false
		for _, account := range conversation.OtherAccounts {
			// TODO: (Vyr) this should use the visibility cache
			...
			if err != nil {
				err = gtserror.Newf(
					"couldn't check blocks between accounts %s, %s: %w",
					requestingAccount.ID,
					account.ID,
					err,
				)
			...
			if compiledMutes.Matches(account.ID, statusfilter.FilterContextNotifications, now) {
				continue
			}

			// Account is not suspended, blocked, or muted.
			atLeastOneAccountVisible = true
			break
		}
		if !atLeastOneAccountVisible {
			continue
		}
