...
func (f *DB) ReplyRequest(ctx context.Context, replyReq vocab.GoToSocialReplyRequest) error {
	...
	if err != nil {
		return gtserror.NewfWithCode(
			http.StatusBadRequest,
			"invalid status attributedTo value: %w", err,
		)
	}
...
func (f *DB) AnnounceRequest(ctx context.Context, announceReq vocab.GoToSocialAnnounceRequest) error {
	...
	// Ensure announce received by correct account.
	// This also ensures it's an announce targeting
	// one of our accounts on this instance.
	if boost.BoostOfAccountID != fpir.receiving.ID {
		err := gtserror.NewfWithCode(
			http.StatusForbidden,
			"receiver %s is not expected %s",
			fpir.receiving.URI, boost.BoostOfAccount.URI,
		)
		return err
	}
