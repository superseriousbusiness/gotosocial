...
func NormalizeOutgoingInteractionPolicyProp(item WithInteractionPolicy, rawJSON map[string]interface{}) {
	...
	for _, rulesKey := range []string{
		...
		for _, PolicyURIsKey := range []string{
			"always",
			"approvalRequired",
		} {
			PolicyURIsVal, ok := rulesValMap[PolicyURIsKey]
			if !ok {
				// Not set.
				continue
			}

			if _, ok := PolicyURIsVal.([]interface{}); ok {
				// Already slice,
				// nothing to change.
				continue
			}
			...
			// Coerce single-object to slice.
			rulesValMap[PolicyURIsKey] = []interface{}{PolicyURIsVal}
		}
	}
}
