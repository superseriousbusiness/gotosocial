...
// ErrorSMTP is returned from the email sender when
// an smtp-specific error has occurred. The wrapped
// error will always have been generated by a call
// to the smtp.SendMail() function.
type ErrorSMTP struct {
	wrapped error
}

func (e *ErrorSMTP) Error() string {
	return "SMTP error: " + e.wrapped.Error()
}

func (e *ErrorSMTP) Unwrap() error {
	return e.wrapped
}
