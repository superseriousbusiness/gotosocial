...
type StatusFave struct {
	...
	AccountID       string    `validate:"required,ulid" bun:"type:CHAR(26),unique:statusfavesrctarget,nullzero,notnull"` // id of the account that created ('did') the fave
	Account         *Account  `validate:"-" bun:"-"`                                                                     // account that created the fave
	TargetAccountID string    `validate:"required,ulid" bun:"type:CHAR(26),unique:statusfavesrctarget,nullzero,notnull"` // id the account owning the faved status
