...
func (s *statusFaveDB) DeleteStatusFaves(ctx context.Context, targetAccountID string, originAccountID string, statusID string) db.Error {
	if targetAccountID == "" && originAccountID == "" && statusID == "" {
		return errors.New("DeleteStatusFaves: one of targetAccountID, originAccountID, or statusID must be set")
	}
	...
	if statusID != "" {
		q = q.Where("? = ?", bun.Ident("status_fave.status_id"), statusID)
	}
