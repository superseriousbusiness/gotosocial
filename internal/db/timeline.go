...
type Timeline interface {
	...
	GetHomeTimeline(ctx context.Context, accountID, maxID, sinceID, minID string, limit int, local bool) ([]*gtsmodel.Status, Error)

	// GetPublicTimeline fetches the account's PUBLIC timeline -- ie., posts and replies that are public.
	// It will use the given filters and try to return as many statuses as possible up to the limit.
	//
	// Statuses should be returned in descending order of when they were created (newest first).
	GetPublicTimeline(ctx context.Context, maxID, sinceID, minID string, limit int, local bool) ([]*gtsmodel.Status, Error)
	...
	GetFavedTimeline(ctx context.Context, accountID, maxID, minID string, limit int) ([]*gtsmodel.Status, string, string, Error)
}
