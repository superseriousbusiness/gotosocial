...
func (m *Module) InstancePeersGETHandler(c *gin.Context) {
	...
	if includeBlocked && !config.GetInstanceExposeBlocklist() && isUnauthenticated {
	...
	if includeAllowed && !config.GetInstanceExposeAllowlist() && isUnauthenticated {
	...
	if flatStr := c.Query(PeersFlatKey); !flatten && flatStr != "" {
		var err error
