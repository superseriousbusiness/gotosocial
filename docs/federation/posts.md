...
`interactionPolicy` is a property attached to the objects `Note`, `Article`, `Question`, etc, with the following format:
...
An empty array, or a missing or null key, indicates that nobody can do the interaction. For example, the following `canLike` value indicates that nobody can `Like` the post:
...
In cases where a user is present in a Collection URI, and is targeted explicitly by URI, the more specific value takes precedence.
...
Here, `@someone@example.org` is present in the `always` array, and is implicitly present in the magic ActivityStreams Public collection in the `approvalRequired` array. In this case, they can always reply, as the `always` value is more explicit.
...
In case the **exact same** URI is present in both `always` and `approvalRequired`, the more permissive value takes precedence (ie., `always`).

### Implicit Assumptions

GoToSocial makes several implicit assumptions about `interactionPolicy`s.

Firstly, users [mentioned](#mentions) in a post should **ALWAYS** be able to reply to the post without requiring approval, regardless of the post visiblity and the `interactionPolicy`. This is to prevent a would-be harasser from mentioning someone in an abusive post, and leaving no recourse to the mentioned user to reply.
...
Secondly, a user should **ALWAYS** be able to reply to their own post, like their own post, and boost their own post without requiring approval.
...
`approvedBy` is an additional property added to the `Create`, `Like`, and `Announce` activities, which signals that the author of the post targeted by the `Activity` has approved/accepted the interaction, and it can now be distributed to its intended audience, as indicated by the `to` and `cc` fields of the `Activity`. The value of `approvedBy` should be the URI of the `Accept` `Activity` created by the author of the post being interacted with.
...
When receiving an `Activity` with an `approvedBy` value attached to it, remote instances should dereference the URI value of the field to validate that the `Accept` `Activity` exists and has an `object` value equal to the `id` of the interaction `Activity`. If the `Accept` cannot be dereferenced, or points to a different `object` than the interaction `Activity`, the interaction should be dropped.

As a consequence of this mechanism, instances should make sure that they serve a valid ActivityPub Object in response to dereferences of `Accept` URIs that pertain to an `interactionPolicy`. If they do not, they inadvertently severely restrict the ability of remote instances to distribute their posts.
...
To avoid this issue, it is recommended that remote instances prevent users from being able to widen scope.
