...
// In an ordered collection (or an ordered collection page) in which items (or
// orderedItems) contains a continuous subset of the collection’s items from
// one end, earlyItems identifiers a continuous subset from the other end. For
// example, if items lists the chronologically latest items, earlyItems would
// list the chrologically earliest items. The ordering rule for items in
// earlyItems MUST be the same as in items. For examle, if items lists items
// in reverse chronogical order, then so does earlyItems.
//
//   {
//     "@context": [
